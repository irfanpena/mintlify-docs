...
1. **Input Detected**  
   The AI receives user inputs in real time during the conversation.
...
4. **Toolcall Broadcast**  
   Tavus broadcasts a [tool call](https://docs.tavus.io/sections/event-schemas/conversation-toolcall) event over the active [Daily](https://docs.daily.co) room. Your app can listen for this event, handle the tool call (e.g. by calling an API), and return the result to the AI for use in its response. 
