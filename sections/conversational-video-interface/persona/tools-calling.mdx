...
1. **Input Detected**  
   ...
   <br/>**Example**: The user says, `“What time is it now in the USA?”`

2. **Tool Matching**  
   The LLM analyzes the input and identifies that the user's question matches the purpose of the `get_current_time` function. This function is defined in your LLM tool configuration to retrieve the current local time for a given location.
...
3. **`tool_call` Broadcast**  
   ...
   <br/>`“It’s currently 2:43 PM in New York, USA.”`
